...
#
# ............modified code to easily get the desired results on SMART dataset..............
#
import argparse
import json
import sys
...
def json_dump(pred_triples, id2wikidata):
  sort_idxs = sorted(range(len(pred_triples)), key=lambda idx: pred_triples[idx][1])
  pred = []
  for idx in sort_idxs:
    if pred_triples[idx][0] in id2wikidata:
        pred.append(id2wikidata[pred_triples[idx][0]])
  return pred
...
def run(
    ...
    if args.interactive:
        ...
        print(len(samples), len(nns), len(dists), len(pred_mention_bounds), len(cand_scores), len(mention_scores))
        ...
        list_wikidata = []
        for i in range(0, len(all_entity_preds)):
          pred_triples = all_entity_preds[i]['pred_triples']
          pred = json_dump(pred_triples, id2wikidata) 
          dict1 = {
            "id":i,
            "wikidata": pred,
          }
          list_wikidata.append(dict1)
        jsonString = json.dumps(list_wikidata)
